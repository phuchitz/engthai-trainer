          ...
          patterns: [
            {
              group: ["@/lib/db/*", "@/lib/db"],
              message:
                "Pure domain modules take values in and return values out; persistence belongs to repositories.",
            },
            {
              group: ["@/components/*", "@/stores/*", "@/hooks/*", "@/app/*"],
              message: "Pure domain modules must not depend on the UI layer.",
            },
          ],
        },
      ],
      ...
      "no-restricted-imports": [
        "error",
        {
          patterns: [
            {
              group: ["@/components/*", "@/stores/*", "@/hooks/*", "@/app/*"],
              message: "Repositories are the persistence boundary and must not depend on the UI layer.",
            },
          ],
        },
      ],
    ...
    rules: {
      "no-restricted-imports": [
        "error",
        {
          patterns: [
            {
              group: ["idb", "@/lib/db/client"],
              message:
                "Go through a repository in src/lib/db/repositories instead of opening the database directly.",
            },
          ],
        },
      ],
